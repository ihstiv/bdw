...
{	
	...
	public static function defaultConstants()
	{
		...
		if ( isset( $_SERVER['IPS_CIC'] ) )
		{
			...
			{
				$uniqueKey = preg_match( '/^\/var\/www\/html\/(.+?)$/i', __DIR__, $matches ) ? str_replace( '/', '', $matches[1] ) : mb_substr( md5( 'abf7b' . '$Rev: 3023$'), 10, 10 );
			}
		}
		else
		{
			$uniqueKey = mb_substr( md5( '8fcf3' . '$Rev: 3023$'), 10, 10 );
		}
		...
		return array(
			...
			// to change them.
				...
				'CACHEBUST_KEY'		=> mb_substr( md5( '8fcf3' . '$Rev: 3023$'), 10, 10 ),
	...
	public static function licenseKey( $forceRefresh = FALSE )
	{
		...
		if ( $response === NULL )
		{
			try
			{
				...
				\IPS\Data\Store::i()->license_data	= array( 'fetched' => time() - 1813800, 'data' => $cached['data'] ?? NULL );
				...
				{
					\IPS\Data\Store::i()->license_data	= array( 'fetched' => time() - 1728000, 'data' => $cached['data'] ?? NULL );
					return $cached;
				}
				...
				if ( method_exists( \IPS\core\DataLayer::i(), "clearCachedConfiguration" ) )
				{
