...
(function()
{
	...
	CKEDITOR.plugins.enterkey =
	{
		enterBlock : function( editor, mode, range, forceMode )
		{
			...
			// IPB bug fix - enter between <blockquote> and <cite> in Firefox creates empty quote boxes
			// @link http://community.invisionpower.com/resources/bugs.html/_/ip-board/enter-creates-new-quote-boxes-r40969
			if( range.startContainer.$.nodeName == 'BLOCKQUOTE' && range.startContainer.$.className == 'ipsBlockquote' && block.getFirst().$.nodeName == 'CITE' && block.getFirst().$.className == 'ipb' )
			{
				return;
			}
